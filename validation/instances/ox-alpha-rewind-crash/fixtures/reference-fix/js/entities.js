...
(function () {
  ...
  class EchoGhost {
    constructor(recording, loopNo) {
      ...
      // Placed from the recording immediately: the echo is created inside step(), after the
      // frame's echo updates have run, so it is drawn once before update() would position it.
      const first = Math.max(0, Math.min(0, this.len - 1));
      this.x = this.xs[first]; this.y = this.ys[first]; this.pose = this.ps[first];
    }
